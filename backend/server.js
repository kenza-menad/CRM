...
// REMPLACE le bloc require("./brevo") par :
const {
  setPool,
...
// ✅ Injecter le pool dans brevo pour le logging automatique
setPool(pool);

function safeDbHost(url) {
  try { return new URL(url).host; }
  catch { return (url.split("@")[1] || "").split("/")[0] || "unknown"; }
}
...
app.get("/health", async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ ok: false, error: "DB connection failed", detail: e.message }); }
});
...
app.post("/auth/signup", async (req, res) => {
  try {
    ...
    // ✅ Email bienvenue avec user_id pour le log
    sendWelcomeEmail({ email: user.email, first_name: user.first_name, user_id: user.id });
...
app.post("/auth/login", async (req, res) => {
  try {
    ...
    const r = await pool.query(
      `SELECT id, email, role, password_hash, first_name, last_name, is_active FROM users WHERE email=$1`, [email]
    );
    const user = r.rows[0];
    if (!user) return res.status(401).json({ error: "invalid credentials" });
    // ✅ Vérifier que le compte est actif
    if (user.is_active === false) return res.status(403).json({ error: "Compte désactivé. Contactez un administrateur." });
...
app.get("/contacts", requireAuth, async (req, res) => {
  try {
    ...
    if (companyId) { params.push(companyId); where.push(`c.company_id = $${i}`); i++; }
    const r = await pool.query(`
...
// ✅ Historique emails d'un contact
app.get("/contacts/:id/emails", requireAuth, async (req, res) => {
  try {
    const r = await pool.query(`
...
app.post("/leads", requireAuth, async (req, res) => {
  try {
    ...
    res.status(201).json(r.rows[0]);
    // ✅ Email nouveau lead assigné
    if (assigned_to) {
      try {
        const u = await pool.query(`SELECT id, email, first_name FROM users WHERE id=$1`, [assigned_to]);
        if (u.rows[0]) {
          sendNewLeadEmail({
...
app.patch("/leads/:id/status", requireAuth, async (req, res) => {
  try {
    ...
    res.json(r.rows[0]);
    // ✅ Email lead converti
    if (status === "converti" && r.rows[0].assigned_to) {
      try {
        const u = await pool.query(`SELECT id, email, first_name FROM users WHERE id=$1`, [r.rows[0].assigned_to]);
        if (u.rows[0]) {
          sendLeadConvertedEmail({
            email: u.rows[0].email,
            first_name: u.rows[0].first_name,
            lead_title: r.rows[0].title,
            value_eur: r.rows[0].value_eur,
            user_id: u.rows[0].id,
          });
        }
      } catch (emailErr) { console.error("Erreur email lead converti:", emailErr.message); }
    }
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post("/tasks", requireAuth, async (req, res) => {
  try {
    ...
    if (assigned_to) {
      ...
      } catch (emailErr) { console.error("Erreur email tâche assignée:", emailErr.message); }
    }
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.patch("/tasks/:id", requireAuth, async (req, res) => {
  try {
    ...
    res.json(task);
    // ✅ Email tâche en retard
    if (task.assigned_to && !task.done && task.due_at && new Date(task.due_at) < new Date()) {
      try {
        const u = await pool.query(`SELECT id, email, first_name FROM users WHERE id=$1`, [task.assigned_to]);
        if (u.rows[0]) {
          sendOverdueTaskEmail({
            email: u.rows[0].email,
            first_name: u.rows[0].first_name,
...
// ✅ Changer le rôle → email automatique
app.patch("/users/:id/role", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    } catch (emailErr) { console.error("Erreur email rôle:", emailErr.message); }
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ✅ Activer/désactiver compte → email automatique
app.patch("/users/:id/active", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    } catch (emailErr) { console.error("Erreur email statut:", emailErr.message); }
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/users/:id", requireAuth, async (req, res) => {
  try {
    ...
    const r = await pool.query(
      `SELECT id, first_name, last_name, email, role, phone, job_title, created_at FROM users WHERE id=$1`, [req.params.id]
    );
...
app.get("/dashboard/stats", requireAuth, async (req, res) => {
  try {
    ...
    if (start && end) {
      ...
      if (p === "today") {
        startDate = endDate;
        prevStartDate = new Date(now.getTime() - 86400000).toISOString().split("T")[0]; prevEndDate = startDate;
      } else if (p === "7d") {
        startDate = new Date(now.getTime() - 7 * 86400000).toISOString().split("T")[0];
        prevStartDate = new Date(now.getTime() - 14 * 86400000).toISOString().split("T")[0]; prevEndDate = startDate;
      } else if (p === "30d") {
        startDate = new Date(now.getTime() - 30 * 86400000).toISOString().split("T")[0];
        prevStartDate = new Date(now.getTime() - 60 * 86400000).toISOString().split("T")[0]; prevEndDate = startDate;
      } else if (p === "month") {
        startDate = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().split("T")[0];
        prevStartDate = new Date(now.getFullYear(), now.getMonth() - 1, 1).toISOString().split("T")[0]; prevEndDate = startDate;
      } else {
        startDate = new Date(now.getFullYear(), 0, 1).toISOString().split("T")[0];
        prevStartDate = new Date(now.getFullYear() - 1, 0, 1).toISOString().split("T")[0]; prevEndDate = startDate;
      }
    }
    ...
    const [
      ...
      pool.query(`SELECT COALESCE(SUM(CASE WHEN status='gagne' THEN amount END),0)::numeric AS ca_gagne, COALESCE(SUM(CASE WHEN status NOT IN ('gagne','perdu') THEN amount END),0)::numeric AS pipeline_total, COUNT(*)::int AS total_deals, COUNT(CASE WHEN status='gagne' THEN 1 END)::int AS deals_gagnes, COUNT(CASE WHEN status='perdu' THEN 1 END)::int AS deals_perdus, COUNT(CASE WHEN status='prospect' THEN 1 END)::int AS deals_prospect, COUNT(CASE WHEN status='qualification' THEN 1 END)::int AS deals_qualification, COUNT(CASE WHEN status='proposition' THEN 1 END)::int AS deals_proposition, COUNT(CASE WHEN status='negociation' THEN 1 END)::int AS deals_negociation FROM deals WHERE created_at::date BETWEEN $1 AND $2`, [startDate, endDate]),
      pool.query(`SELECT COALESCE(SUM(CASE WHEN status='gagne' THEN amount END),0)::numeric AS ca_gagne, COUNT(*)::int AS total_deals, COUNT(CASE WHEN status='gagne' THEN 1 END)::int AS deals_gagnes FROM deals WHERE created_at::date BETWEEN $1 AND $2`, [prevStartDate, prevEndDate]),
      pool.query(`SELECT COUNT(*)::int AS total_leads, COUNT(CASE WHEN status='nouveau' THEN 1 END)::int AS leads_nouveau, COUNT(CASE WHEN status='en_cours' THEN 1 END)::int AS leads_en_cours, COUNT(CASE WHEN status='converti' THEN 1 END)::int AS leads_converti, COUNT(CASE WHEN status='perdu' THEN 1 END)::int AS leads_perdu FROM leads WHERE created_at::date BETWEEN $1 AND $2`, [startDate, endDate]),
      pool.query(`SELECT COUNT(*)::int AS total_leads FROM leads WHERE created_at::date BETWEEN $1 AND $2`, [prevStartDate, prevEndDate]),
      pool.query(`SELECT COUNT(*)::int AS total_tasks, COUNT(CASE WHEN done=false THEN 1 END)::int AS tasks_todo, COUNT(CASE WHEN done=false AND due_at < now() THEN 1 END)::int AS tasks_overdue FROM tasks`),
      pool.query(`SELECT d.id, d.title, d.status, d.amount, c.first_name, c.last_name FROM deals d LEFT JOIN contacts c ON c.id = d.contact_id WHERE d.created_at::date BETWEEN $1 AND $2 ORDER BY d.updated_at DESC LIMIT 5`, [startDate, endDate]),
      pool.query(`SELECT l.id, l.title, l.status, l.value_eur, c.first_name, c.last_name FROM leads l LEFT JOIN contacts c ON c.id = l.contact_id WHERE l.created_at::date BETWEEN $1 AND $2 ORDER BY l.created_at DESC LIMIT 5`, [startDate, endDate]),
      pool.query(`SELECT t.id, t.title, t.due_at, t.done, COALESCE(t.priority,'normal') AS priority FROM tasks t WHERE t.done=false AND (t.due_at::date <= CURRENT_DATE OR t.priority='urgent') ORDER BY t.priority DESC, t.due_at ASC NULLS LAST LIMIT 5`),
      pool.query(`SELECT TO_CHAR(DATE_TRUNC('month',created_at),'YYYY-MM') AS month, COUNT(*)::int AS count, COALESCE(SUM(CASE WHEN status='gagne' THEN amount END),0)::numeric AS total FROM deals WHERE created_at >= NOW() - INTERVAL '12 months' GROUP BY DATE_TRUNC('month',created_at) ORDER BY DATE_TRUNC('month',created_at)`),
      pool.query(`SELECT status, COUNT(*)::int AS count, COALESCE(SUM(amount),0)::numeric AS total FROM deals WHERE created_at::date BETWEEN $1 AND $2 GROUP BY status`, [startDate, endDate]),
      pool.query(`SELECT status, COUNT(*)::int AS count FROM deals GROUP BY status`),
      pool.query(`SELECT u.id, u.first_name, u.last_name, COUNT(d.id)::int AS total_deals, COUNT(CASE WHEN d.status='gagne' THEN 1 END)::int AS deals_gagnes, COALESCE(SUM(CASE WHEN d.status='gagne' THEN d.amount END),0)::numeric AS ca, COUNT(t.id)::int AS tasks_open FROM users u LEFT JOIN deals d ON d.assigned_to=u.id AND d.created_at::date BETWEEN $1 AND $2 LEFT JOIN tasks t ON t.assigned_to=u.id AND t.done=false GROUP BY u.id, u.first_name, u.last_name ORDER BY ca DESC LIMIT 5`, [startDate, endDate]),
    ]);

    const d = dealsStats.rows[0]; const dp = dealsPrev.rows[0];
    const l = leadsStats.rows[0]; const lp = leadsPrev.rows[0];
    ...
    const byStatus = {}; dealsByStatus.rows.forEach(r => { byStatus[r.status] = { count: r.count, total: Number(r.total) }; });
    const byStatusGlobal = {}; dealsByStatusGlobal.rows.forEach(r => { byStatusGlobal[r.status] = r.count; });
    ...
    res.json({
      ...
      deals_by_status: { prospect: byStatus["prospect"]?.count ?? 0, qualification: byStatus["qualification"]?.count ?? 0, proposition: byStatus["proposition"]?.count ?? 0, negociation: byStatus["negociation"]?.count ?? 0, gagne: byStatus["gagne"]?.count ?? 0, perdu: byStatus["perdu"]?.count ?? 0 },
      deals_by_status_global: { prospect: byStatusGlobal["prospect"] ?? 0, qualification: byStatusGlobal["qualification"] ?? 0, proposition: byStatusGlobal["proposition"] ?? 0, negociation: byStatusGlobal["negociation"] ?? 0, gagne: byStatusGlobal["gagne"] ?? 0, perdu: byStatusGlobal["perdu"] ?? 0 },
      leads_by_status: { nouveau: l.leads_nouveau, en_cours: l.leads_en_cours, converti: l.leads_converti, perdu: l.leads_perdu },
      recent_deals: recentDeals.rows, recent_leads: recentLeads.rows,
      today_tasks: todayTasks.rows, monthly_sales: dealsByMonth.rows, commerciaux: commerciaux.rows,
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/dashboard/activity", requireAuth, async (req, res) => {
  try {
    const [deals, leads, tasks, companies, contacts] = await Promise.all([
      pool.query(`SELECT d.id, d.title, d.updated_at AS at, u.first_name, u.last_name FROM deals d LEFT JOIN users u ON u.id = d.assigned_to ORDER BY d.updated_at DESC LIMIT 5`),
      pool.query(`SELECT l.id, l.title, l.created_at AS at, u.first_name, u.last_name FROM leads l LEFT JOIN users u ON u.id = l.assigned_to ORDER BY l.created_at DESC LIMIT 5`),
      pool.query(`SELECT t.id, t.title, t.created_at AS at, u.first_name, u.last_name FROM tasks t LEFT JOIN users u ON u.id = t.assigned_to ORDER BY t.created_at DESC LIMIT 5`),
      pool.query(`SELECT co.id, co.name AS title, co.created_at AS at, NULL AS first_name, NULL AS last_name FROM companies co ORDER BY co.created_at DESC LIMIT 5`),
      pool.query(`SELECT c.id, CONCAT(c.first_name,' ',c.last_name) AS title, c.created_at AS at, NULL AS first_name, NULL AS last_name FROM contacts c ORDER BY c.created_at DESC LIMIT 5`),
    ]);
...
app.get("/dashboard/my-activity", requireAuth, async (req, res) => {
  try {
    ...
    const [deals, leads, tasks, contacts] = await Promise.all([
      pool.query(`SELECT d.id, d.title, d.status, d.amount, d.updated_at FROM deals d WHERE d.assigned_to=$1 ORDER BY d.updated_at DESC LIMIT 4`, [userId]),
      pool.query(`SELECT l.id, l.title, l.status, l.value_eur, l.created_at FROM leads l WHERE l.assigned_to=$1 ORDER BY l.created_at DESC LIMIT 4`, [userId]),
      pool.query(`SELECT t.id, t.title, t.due_at, t.done FROM tasks t WHERE t.assigned_to=$1 AND t.done=false ORDER BY t.due_at ASC NULLS LAST LIMIT 4`, [userId]),
      pool.query(`SELECT id, first_name, last_name, created_at FROM contacts ORDER BY created_at DESC LIMIT 4`),
    ]);
    res.json({ my_deals: deals.rows, my_leads: leads.rows, my_tasks: tasks.rows, my_contacts: contacts.rows });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ✅ ---------- Emails historique (admin) ----------
app.get("/emails/history", requireAuth, requireAdmin, async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 50;
    const type  = req.query.type || null;
    const where = type ? "WHERE e.type = $2" : "";
...
// ✅ Stats emails pour le dashboard
app.get("/emails/stats", requireAuth, requireAdmin, async (req, res) => {
  try {
    const [totals, byType] = await Promise.all([
      pool.query(`
        SELECT COUNT(*)::int AS total_sent,
               COUNT(CASE WHEN status='failed' THEN 1 END)::int AS total_failed,
               COUNT(CASE WHEN sent_at >= NOW() - INTERVAL '30 days' THEN 1 END)::int AS sent_last_30d,
               COUNT(DISTINCT to_email)::int AS unique_recipients
        FROM emails_log
      `),
      pool.query(`SELECT type, COUNT(*)::int AS count FROM emails_log GROUP BY type ORDER BY count DESC`),
    ]);
    res.json({ ...totals.rows[0], by_type: byType.rows });
  } catch (e) { res.status(500).json({ error: e.message }); }
});


// ✅ ---------- Envoi email manuel depuis fiche contact ----------
app.post("/emails/send", requireAuth, async (req, res) => {
  try {
    const { to_email, to_name, subject, body, contact_id } = req.body;
    if (!to_email?.trim() || !subject?.trim() || !body?.trim())
      return res.status(400).json({ error: "to_email, subject et body sont requis" });
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
          ...
          <div style="background:#fff;padding:32px;border-radius:0 0 12px 12px;border:1px solid #e2e8f0">
            ${to_name ? `<p style="color:#64748b;font-size:14px">À : <strong>${to_name}</strong></p>` : ""}
            <div style="white-space:pre-wrap;color:#1e293b;line-height:1.7;font-size:15px">${body.trim()}</div>
            <p style="color:#94a3b8;font-size:11px;margin-top:32px;border-top:1px solid #f1f5f9;padding-top:16px">
              Envoyé via FormaPro CRM
            </p>
          </div>
        </div>`,
    });
...
app.get("/emails/templates", requireAuth, async (req, res) => {
  try {
    const r = await pool.query(`SELECT * FROM email_templates ORDER BY created_at DESC`);
    res.json(r.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post("/emails/templates", requireAuth, async (req, res) => {
  try {
    const { name, subject, body } = req.body;
    if (!name?.trim() || !subject?.trim() || !body?.trim())
      return res.status(400).json({ error: "name, subject et body requis" });
    const r = await pool.query(
...
app.put("/emails/templates/:id", requireAuth, async (req, res) => {
  try {
    const { name, subject, body } = req.body;
    if (!name?.trim() || !subject?.trim() || !body?.trim())
      return res.status(400).json({ error: "name, subject et body requis" });
    const r = await pool.query(
      ...
      [name.trim(), subject.trim(), body.trim(), req.params.id]
    );
    if (r.rowCount === 0) return res.status(404).json({ error: "Template introuvable" });
    res.json(r.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.delete("/emails/templates/:id", requireAuth, async (req, res) => {
  try {
